...
public class RecipeBook {
	private ArrayList < RecipeCategory > recipecategories = new
	ArrayList < RecipeCategory > ();
	...
	private void CreateTempCategories() {
		...
		recipecategories.add(new RecipeCategory("Category"));
	}
	...
	UnitOfMeasure uom, String ingredient) {
		...
		if (recipe != null) { //violation – creator!!
	...
	public void addInstruction(String recipename, String instruction) {
		Recipe recipe = getRecipe(recipename);
		if (recipe != null) { //violation – creator pattern!!
			// violation – low coupling!!
			// violation – high cohesion!!
	...
	public String printRecipe(Recipe r) {
		...
		if (r != null) {
			...
			for (int i = 0; i < r.numIngredients(); i++) {
				...
				if (ing.getUOM() == null) s += s.format("%.2f %s\n", ing.getUnit(),
				ing.getIngredient());
				else s += s.format("%.2f %s %s\n", ing.getUnit(),
				ing.getUOM(), ing.getIngredient());
			}
			s += s.format("\nINSTRUCTIONS\n");
